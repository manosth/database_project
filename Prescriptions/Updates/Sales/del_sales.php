    ...
    <body>
        <?php
            if (isset($_POST['submit'])) {
                ...
                $sql = "DELETE FROM Sell WHERE PharmacyId =" . substr($todelete, 0, 1) . " AND DrugId =" . substr($todelete, 1, 1);
        ...
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']?>">
            ...
            <?php
                ...
                $sql = "SELECT Pharmacy.PharmacyId, Drug.DrugId, Pharmacy.Name AS PharmacyName, Drug.Name
                FROM Sell, Pharmacy, Drug
                WHERE Pharmacy.PharmacyId = Sell.PharmacyId AND Drug.DrugId = Sell.DrugId";
                $result = $conn->query($sql);

                $option = "<select class='selectpicker' name='sale'>";
                if ($result->num_rows > 0 ) {
                    while($row = $result->fetch_assoc()) {
                        $option .= "<option value=" . $row['PharmacyId'] . $row['DrugId'] . ">" . $row['PharmacyName'] . ", " . $row['Name'] . "</option>";
                    }
                }
                $option .= "</select>";
